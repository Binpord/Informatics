...
int catchfile(char** args, int count);
...
int ml_bash()
{
	int pid = 0, stat = 0, check = 0, ret = 0;
	...
	while(true)
	{
		...
		stat = execute(newargv, STDIN_FILENO, &procs);
		/*if(stat == BAD_CHILD)
		{
			proc = *(list_findlast(&procs)->info);
			check = list_cut(&procs, &proc, proc.pid);
			if(check == FAILURE) 
			{
...
int execute(char** newargv, int newinfd, list_t* procs)
{
	...
	int pid = 0;
	int fd[2], fSrc, fDst;
	int stat = 0, stat2 = 0, stat3 = 0;
	
	stat = findarg('|', newargv);
	if(stat < 0)				//returns -1 if smbl wasn't detected
	{
		stat2 = findarg('<', newargv);
		if(stat2 < 0)
		{
			stat3 = findarg('>', newargv);
			if(stat3 < 0)
			{
				pid = fork();
				if(pid < 0)
